...
static std::optional<std::vector<std::pair<const Guest *, int>>>
findMostEfficientSubset(const std::unordered_map<const Guest *, int> &unplaced, const Host &host,
                        int subsetSize)
{
    std::vector<std::pair<const Guest *, int>> guests(unplaced.begin(), unplaced.end());
    const int guestCount = static_cast<int>(guests.size());
    subsetSize = std::min(guestCount, subsetSize);

    std::optional<std::vector<std::pair<const Guest *, int>>> bestSubset;
    ...
    do {
        std::vector<std::pair<const Guest *, int>> subset;
        subset.reserve(subsetSize);
        for (const int index : indices) {
            subset.emplace_back(guests[index]);
        }

        std::vector<const Guest *> candidateView;
                                       ...
                                       int initialSubsetSize)
{
    ...
    std::unordered_map<const Guest *, int> unplaced = profits;
...
struct ProfitOption
{
    ClusterTree::ClusterId cid;  // scenario considers the subtree rooted here
...
struct GuestSelection
{
    ...
    GuestSelection(const size_t pageCount, std::vector<const Guest *> guests)
        : pageCount(pageCount), guests(std::move(guests))
    {
    }

    void setFromSelection(const std::vector<ClusterTree::NodeId> &selection,
                          const std::unordered_set<int> &pages, const ClusterTree &tree)
    {
        pageCount = pages.size();
        guests.clear();
        guests.reserve(selection.size());

        for (const ClusterTree::NodeId selected : selection) {
...
static std::pair<std::vector<ClusterTree::NodeId>, std::unordered_set<int>>
selectNodesByMask(const ClusterTree &tree, const std::vector<ClusterTree::NodeId> &pool,
                  const uint64_t mask)
{
    std::unordered_set<int> pages;
    std::vector<ClusterTree::NodeId> selection;
...
static size_t makeAccessibleChildrenMask(const std::vector<ClusterTree::NodeId> &children,
                                         const std::vector<ClusterTree::NodeId> &allowedParents,
                                         const ClusterTree &tree)
{
    ...
    for (size_t i = 0; i < children.size(); ++i) {
        ...
        for (const ClusterTree::NodeId parentCandidate : allowedParents) {
...
findLowestCostAccessibleSelection(const auto &costs, const ClusterTree::ClusterId cid,
                                  ...
                                  const ClusterTree &tree)
{
    ...
    for (uint64_t selectionMask = 0; selectionMask < 1ULL << nodes.size(); ++selectionMask) {
        ...
        const size_t degree = tree.childrenOfCluster(cid).size();
...
static std::optional<GuestSelection>
findMostProfitableScenarioAtRoot(const auto &costs, const ClusterTree &tree, const size_t capacity)
{
    using ClusterId = ClusterTree::ClusterId;

    const ClusterId rootCid = tree.rootCluster();
                                  ...
                                  const std::unordered_map<const Guest *, int> &profits)
{
    ...
    std::unordered_map<ClusterId, size_t> profitUpperBounds;
    ...
    std::unordered_map<ClusterId, size_t> unvisitedClusterChildCount;
    std::queue<ClusterId> clustersToVisit;

    for (ClusterId cid = 0; cid < tree.clusterCount(); ++cid) {
        const size_t childCount = tree.childrenOfCluster(cid).size();

        if ((unvisitedClusterChildCount[cid] = childCount) == 0) {
            clustersToVisit.push(cid);
        }
    }

    while (!clustersToVisit.empty()) {
        const ClusterId cid = clustersToVisit.front();
        // We consider one cluster at a time, bottom-up
        clustersToVisit.pop();

        // Decrement the unvisited child count of each parent
        const ClusterId parentCid = tree.parentOfCluster(cid);
        if (--unvisitedClusterChildCount[parentCid] == 0) {
            clustersToVisit.push(parentCid);
        }

        const auto &curNodes = tree.nodesOfCluster(cid);
        const auto &curChildren = tree.childrenOfCluster(cid);

        if (tree.isLeafCluster(cid)) {
            const auto &guest = tree.guestOfNode(curNodes.front());
            profitUpperBounds[cid] = profits.at(guest);
        }
        else {
            for (const ClusterId childCid : curChildren) {
                profitUpperBounds[cid] += profitUpperBounds.at(childCid);
            }
        }
        ...
        for (uint64_t curMask = 0; curMask < 1ULL << curNodes.size(); ++curMask) {
            ...
            size_t profitMade = 0;
            for (const NodeId nid : curSelection) {
                if (tree.isLeafNode(nid)) {
                    profitMade += profits.at(tree.guestOfNode(nid));
                }
            }

            for (size_t profitTarget = 0; profitTarget <= profitUpperBounds[cid]; ++profitTarget) {
                // Initialise:
                // cost[n,s,0,p] = if (sum profit in s) >= p then |union pages in s| else +inf
                ProfitOption curKey{ cid, curMask, 0, profitTarget };
                ...
                for (size_t j = 1; j <= curChildren.size(); ++j) {
                    ...
                    curKey = { cid, curMask, j, profitTarget };
                    // Try to do better than with j - 1 children
                    costs[curKey] = costs.at({ cid, curMask, j - 1, profitTarget });
                    ...
                    const ClusterId newChildCid = curChildren[j - 1];
                    const auto &newChildNodes = tree.nodesOfCluster(newChildCid);
                    ...
                    for (size_t profitComplement = 0;
                         profitComplement <=
                         std::min(profitTarget, profitUpperBounds.at(newChildCid));
                         ++profitComplement) {
                        const auto bestChildCost =
                            findLowestCostAccessibleSelection(
                                costs, newChildCid, accessibleChildrenMask, profitComplement, tree)
                                .value_or(GuestSelection());
                        const auto &prevCost =
                            costs.at({ cid, curMask, j - 1, profitTarget - profitComplement });
